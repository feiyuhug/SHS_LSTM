...
class RNNI(nn.Module):
    ...
    def LSTMCell(self, input, hidden, w_ih, w_hh, b_ih=None, b_hh=None, hard_thresh=0.0, norm_thresh=0.0, dropout_state=None):
        ...
        ## slice the input and weights
        input_mask, hx_mask = input > 0, hx > 0
        input_mask, hx_mask = input_mask[0], hx_mask[0]
        input, hx = input[:, input_mask], hx[:, hx_mask]
        w_ih_, w_hh_  = w_ih[:, input_mask], w_hh[:, hx_mask]
        gates = F.linear(input, w_ih_, b_ih) + F.linear(hx, w_hh_, b_hh)
        '''
