...
class RNN(nn.Module):
    ...
    def LSTMCell(self, input, hidden, w_ih, w_hh, b_ih=None, b_hh=None, hard_thresh=0.0, norm_thresh=0.0, dropout_state=None):
        ...
        '''
        gates = F.linear(input, w_ih[0:self.hidden_size * 3], b_ih[0:self.hidden_size * 3]) + F.linear(hx, w_hh[0:self.hidden_size * 3], b_hh[0:self.hidden_size * 3])
        ingate, forgetgate, cellgate = gates.chunk(3, 1)
        outgate = F.linear(input, w_ih[self.hidden_size * 3:], None) + F.linear(hx, w_hh[self.hidden_size * 3:], None)
        '''
        ingate = F.sigmoid(ingate)
        forgetgate = F.sigmoid(forgetgate)
        cellgate = F.tanh(cellgate)
        outgate = F.sigmoid(outgate)
        if len(self.dropout_state.keys()) == 0:
            self.dropout_state[0] = []
        
        '''
        self.dropout_state[0].append(outgate.data.cpu().numpy())
        if len(self.dropout_state[0]) == 351:
            self.dropout_state[0] = []
        if len(self.dropout_state[0]) < 350:
            self.dropout_state[0].append(outgate.data.cpu().numpy())
        elif len(self.dropout_state[0]) == 350:
            np.save(open('np_dump/outgate_%.2f_decay0.0000001_x2.0.np'%(hard_thresh), 'w'), self.dropout_state[0])
            self.dropout_state[0].append(1)
            self.dropout_state[0].append(1)
        '''
        ...
        '''
        out_cond = outgate != 0
        out_cond = out_cond.float()
        forgetgate = forgetgate * out_cond + (1 - out_cond)
        ingate = ingate * out_cond
        '''
    ...
    def forward(self, input, hx=None):
        ...
        if hx is None:
            hx1 = torch.autograd.Variable(input.data.new(self.num_layers,
                                                        max_batch_size,
                                                        self.hidden_size).zero_(),
                                                        requires_grad=False)
            hx2 = torch.autograd.Variable(input.data.new(self.num_layers,
                                                        max_batch_size,
                                                        self.hidden_size).zero_(),
                                                        requires_grad=False)
